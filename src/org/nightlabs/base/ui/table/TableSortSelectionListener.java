...
public class TableSortSelectionListener implements SelectionListener {
     ...
     * @param defaultDirection the default sorting direction of this Listener. Possible values are {@link SWT#UP} and {@link SWT#DOWN}
     */
    public TableSortSelectionListener(TableViewer viewer, TableColumn column, InvertableSorter<?> sorter, int defaultDirection) {
     ...
     * @param sorter the sorter this listener uses
     * @param defaultDirection the default sorting direction of this Listener. Possible values are {@link SWT.UP} and {@link SWT.DOWN}
     * @param keepDirection if true, the listener will remember the last sorting direction of the associated
     *                      column and restore it when the column is reselected.
     *                      If false, the listener will use the default sorting direction
     */
    ...
    /**
     * Chooses the colum of this listener for sorting of the table. Mainly used when first initialising the table.
     */
    ...
    public void widgetSelected(SelectionEvent e) {
        ...
        if (this.viewer.getTable().getSortColumn() == this.column) {
            newSorter = ((InvertableSorter<?>) this.viewer.getSorter()).getInverseSorter();
        } else {
            if (this.keepDirection) {
                newSorter = this.currentSorter;
            } else {
                newSorter = this.sorter;
            }
        }
